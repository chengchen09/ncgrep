...
static cdCalenType
my_calendar_type(int ncid, int varid) {
    int ctype;
    int stat;
    ncatt_t catt;
    static struct {
...
get_timeinfo(int ncid, int varid, ncvar_t *vp) {
	...
	vp->timeinfo->calendar = my_calendar_type(ncid, varid);
...
size_t
my_nc_inq_varname_count(int ncid, char *varname) {
    /* 
       count = 0;
    ...
    */
    ...
    /* Call this function for each group. */
    for (g = 0; g < numgrps; g++) {
	count += my_nc_inq_varname_count(ncids[g], varname);
    }
    free(ncids);
#endif /* USE_NETCDF4 */
    return count;    
...
missing_vars(int ncid, fspec_t *specp) {
    int iv;
    for (iv=0; iv < specp->nlvars; iv++) {
	if(my_nc_inq_varname_count(ncid, specp->lvars[iv]) == 0) {
